...
public class CargoServiceTest {
    ...
    public void deveBuscarCargoPorId() {
        ...
        when(cargoRepository.buscarPorId(id)).thenReturn(Optional.of(cargo));
    ...
    public void deveBuscarCargoPorNome() {
        ...
        when(cargoRepository.buscarPorNome(nome)).thenReturn(cargos);

        // When
        List<Cargo> resultado = cargoService.buscarPorNome(nome);

        // Then
        assertEquals(1, resultado.size());
        assertEquals("Analista de Sistemas", resultado.get(0).getNome());
        verify(cargoRepository).buscarPorNome(nome);
    }
}